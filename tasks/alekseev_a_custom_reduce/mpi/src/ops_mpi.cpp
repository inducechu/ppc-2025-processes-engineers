...
template <typename T>
void ReduceTree(const T* send_data, T* recv_data, int count, int rank, int size, 
                   int root, MPI_Comm comm, MPI_Datatype mpi_type) {
    ...
    for (int step = 1; step < size; step <<= 1) {
        if ((virtual_rank & step) == 0) {
            ...
            if (source_virtual < size) {
                ...
                MPI_Recv(buffer.data(), count, mpi_type, source_rank, 
                        kMessageTag, comm, &status);
            ...
            MPI_Send(accumulator.data(), count, mpi_type, dest_rank, 
                    kMessageTag, comm);
            return;
        }
    }
...
int CustomReduceImpl(const void* send_data, void* recv_data, int count, 
    ...
    if (datatype == MPI_INT) {
        ReduceTree(static_cast<const int*>(send_data), 
                     static_cast<int*>(recv_data), 
                     count, rank, size, root, comm, MPI_INT);
    } else if (datatype == MPI_FLOAT) {
        ReduceTree(static_cast<const float*>(send_data), 
                     static_cast<float*>(recv_data), 
                     count, rank, size, root, comm, MPI_FLOAT);
    } else if (datatype == MPI_DOUBLE) {
        ReduceTree(static_cast<const double*>(send_data), 
                     static_cast<double*>(recv_data), 
                     count, rank, size, root, comm, MPI_DOUBLE);
    } else {
        return MPI_ERR_TYPE;
    }
...
bool AlekseevACustomReduceMPI::PreProcessingImpl() {
    ...
    MPI_Scatterv(global_data, counts.data(), displacements.data(), MPI_DOUBLE,
                local_data_.data(), local_size_, MPI_DOUBLE, 0, MPI_COMM_WORLD);
